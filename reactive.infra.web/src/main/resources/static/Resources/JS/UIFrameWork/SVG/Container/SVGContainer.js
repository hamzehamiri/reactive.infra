...
export default class SVGContainer extends SVGComponent {
    ...
    addItem(component, layoutData, runtimeExecuteLayout) {
        if (layoutData) {
            if (component instanceof HTMLComponent || component instanceof BaseSVGComponent) {
    ...
    isComponentInPoint(event, functionIsPointInPath) {
        for (let item of this.getItems().values()) {
            if (item instanceof SVGComponent) {
                if (item.containElement(event))
                    functionIsPointInPath(item);
            }
        }
    }
