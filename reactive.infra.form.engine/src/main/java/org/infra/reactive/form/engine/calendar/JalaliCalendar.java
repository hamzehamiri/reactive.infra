...
public class JalaliCalendar extends Calendar {
    ...
    public JalaliCalendar(TimeZone zone, Locale aLocale) {
        ...
        EditorDateDTO yearMonthDate = new EditorDateDTO(calendar.get(YEAR), calendar.get(MONTH), calendar.get(DATE));
        yearMonthDate = gregorianToJalali(yearMonthDate);
        set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDayOfMonth());
        complete();
    ...
    JalaliCalendar(int year, int month, int dayOfMonth, int hourOfDay, int minute, int second, int millis) {
        ...
        EditorDateDTO yearMonthDate = jalaliToGregorian(new EditorDateDTO(fields[1], fields[2], fields[5]));
        cal = new GregorianCalendar(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDayOfMonth(), hourOfDay, minute, second);
        time = cal.getTimeInMillis();

        isTimeSeted = true;
    }

    public static EditorDateDTO gregorianToJalali(EditorDateDTO gregorian) {
        return gregorianToJalali(gregorian.getYear(), gregorian.getMonth(), gregorian.getDayOfMonth(), gregorian.getHour(), gregorian.getMinute(), gregorian.getSecond());
    }

    public static EditorDateDTO gregorianToJalali(int year, int month, int dayOfMonth, int hour, int minute, int second) {
        ...
        gregorianDayNo = 365 * year + (int) Math.floor((double) (year + 3) / 4) - (int) Math.floor((double) (year + 99) / 100) + (int) Math.floor((double) (year + 399) / 400);
        for (i = 0; i < month; ++i) {
        ...
        gregorianDayNo += dayOfMonth;

        jalaliDayNo = gregorianDayNo - 79;

        jalaliNP = (int) Math.floor((double) jalaliDayNo / 12053);
        ...
        return new EditorDateDTO(jalaliYear, jalaliMonth, jalaliDay, hour, minute, second);
    }


    public static EditorDateDTO jalaliToGregorian(EditorDateDTO jalali) {
        ...
        jalali.setDayOfMonth(jalali.getDayOfMonth() - 1);
        ...
        jalaliDayNo += jalali.getDayOfMonth();

        gregorianDayNo = jalaliDayNo + 79;

        gregorianYear = 1600 + 400 * (int) Math.floor((double) gregorianDayNo / 146097); /* 146097 = 365*400 + 400/4 - 400/100 + 400/400 */
        gregorianDayNo = gregorianDayNo % 146097;

        leap = 1;
        if (gregorianDayNo >= 36525) /* 36525 = 365*100 + 100/4 */ {
            gregorianDayNo--;
            gregorianYear += 100 * (int) Math.floor((double) gregorianDayNo / 36524); /* 36524 = 365*100 + 100/4 - 100/100 */
        ...
        gregorianYear += 4 * (int) Math.floor((double) gregorianDayNo / 1461); /* 1461 = 365*4 + 4/4 */
        ...
        return new EditorDateDTO(gregorianYear, gregorianMonth, gregorianDay);

    }

    public static int weekOfYear(int dayOfYear, int year) {
        switch (dayOfWeek(JalaliCalendar.jalaliToGregorian(new EditorDateDTO(year, 0, 1)))) {
            case 2:
                dayOfYear++;
                break;
            case 3:
    ...
    public static int dayOfWeek(EditorDateDTO yearMonthDate) {
        Calendar cal = new GregorianCalendar(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDayOfMonth());
    ...
    protected void computeTime() {
        if (!isTimeSet && !isTimeSeted) {
            ...
            EditorDateDTO yearMonthDate = jalaliToGregorian(new EditorDateDTO(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH)));
            cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDayOfMonth(), internalGet(HOUR_OF_DAY), internalGet(MINUTE), internalGet(SECOND));
            time = cal.getTimeInMillis();
        ...
        switch (field) {
            ...
            case DST_OFFSET: {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) && isSet(MINUTE) && isSet(SECOND) && isSet(MILLISECOND)) {
                    cal = new GregorianCalendar();
                    EditorDateDTO yearMonthDate = jalaliToGregorian(new EditorDateDTO(internalGet(YEAR), internalGet(MONTH), internalGet(DATE)));
                    cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDayOfMonth(), internalGet(HOUR_OF_DAY), internalGet(MINUTE), internalGet(SECOND));
                    cal.set(field, value);
                    yearMonthDate = gregorianToJalali(new EditorDateDTO(cal.get(YEAR), cal.get(MONTH), cal.get(DATE)));
                    super.set(YEAR, yearMonthDate.getYear());
                    super.set(MONTH, yearMonthDate.getMonth());
                    super.set(DATE, yearMonthDate.getDayOfMonth());
                    super.set(HOUR_OF_DAY, cal.get(HOUR_OF_DAY));
                    super.set(MINUTE, cal.get(MINUTE));
                    super.set(SECOND, cal.get(SECOND));
    ...
    protected void computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            //Day_of_Week
            super.set(DAY_OF_WEEK, dayOfWeek(jalaliToGregorian(new EditorDateDTO(fields[1], fields[2], fields[5]))));
            //***
    ...
    public void add(int field, int amount) {
        if (field == MONTH) {
            ...
            EditorDateDTO yearMonthDate = jalaliToGregorian(new EditorDateDTO(get(YEAR), get(MONTH), get(DATE)));
            Calendar gc = new GregorianCalendar(yearMonthDate.getYear(), yearMonthDate.getMonth(), yearMonthDate.getDayOfMonth(), get(HOUR_OF_DAY), get(MINUTE), get(SECOND));
            gc.add(field, amount);
            yearMonthDate = gregorianToJalali(new EditorDateDTO(gc.get(YEAR), gc.get(MONTH), gc.get(DATE)));
            super.set(YEAR, yearMonthDate.getYear());
            super.set(MONTH, yearMonthDate.getMonth());
            super.set(DATE, yearMonthDate.getDayOfMonth());
    ...
    public static void main(String[] args) {
        EditorDateDTO dddd = JalaliCalendar.jalaliToGregorian(new EditorDateDTO(1391, 3, 10));
        EditorDateDTO ff = JalaliCalendar.gregorianToJalali(new EditorDateDTO(2024, 3, 29));
        int[] ddddd = DateConverter.gregorian_to_jalali(2024, 1, 9);
        int dd = JalaliCalendar.dayOfWeek(new EditorDateDTO(2024, 1, 9));
